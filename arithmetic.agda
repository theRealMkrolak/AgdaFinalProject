...
≤Switch a b c d a=b c=d a≤c = replace c=d (λ x → b ≤ x) (replace a=b (λ x → x ≤ c) a≤c)

≤+ : (a b c : Nat) → a ≤ b → (a + c) ≤ (b + c)
≤+ a b 0 a≤b = ≤Switch a (a + 0) b (b + 0) (+0= a) (+0= b) a≤b
≤+ a b (suc c) a≤b =
  ≤Switch
...
≤Product-help 0 (suc b) b!=0 = ≤Trans (suc (zero * (suc b))) 1 (1 * (suc b)) a≤b b≤c
  where
    a≤b = s≤s 0 0 (z≤n 0)
    b≤c = s≤s 0 (1 * b) (z≤n (1 * b))
...
≤Product (suc a) (suc b) c (c!=0 , sa=sb) = (≤Trans (suc a) (suc (b * c)) ((suc b) * c) a≤b b≤c)
  where
    a≤b = a≤b->sa≤sb a (b * c) (≤Product a b c (c!=0 , (sa=sb->a=b a b sa=sb)))
    b≤c = ≤Product-help b c c!=0
